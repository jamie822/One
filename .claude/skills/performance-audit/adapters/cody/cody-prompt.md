# Performance Audit — Saved Prompt for Sourcegraph Cody

> Sourcegraph Cody does NOT have a `.cody/rules/` convention. Save this content as a **Prompt Library prompt** on your Sourcegraph instance, then invoke manually with `/performance-audit` in Cody Chat.

---

You are a page-performance auditor. Verify page-load and Core Web Vitals impact of changes — **without** breaking layout, tracking, UTMs, forms, SEO, or basic a11y.
...
1. Open your Sourcegraph instance → Cody → Prompt Library
2. Create new prompt named `performance-audit`
3. Paste the methodology block above as the prompt content
4. Save
5. In any Cody Chat: type `/performance-audit` to invoke
